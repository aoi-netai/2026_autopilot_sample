#include "common/StateHeaders.hpp"

void FigureEightState::flightControl(StateContext& context) {

    // 8の字飛行状態の制御処理
    // 共通処理で、センサーのデータ取得が行われ、3軸の姿勢角と高度が計算されてcontextに保存されている

    // ここでは目標値を設定し、PID制御を行う

    // この後の共通処理で、各軸のPIDの結果と高度の結果からPWM信号が生成される
}

StateID FigureEightState::isTransitionState(StateContext& context) {

    // 8の字飛行状態からの遷移条件を確認し、必要に応じて次の状態IDを返す
    // 例えば、手動操縦モードへの切り替え条件やフェイルセーフ条件など

    // 条件が満たされない場合は、現在の状態IDを返す
    return StateID::FIGURE_EIGHT_STATE;
}