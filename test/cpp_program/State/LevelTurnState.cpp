...
void LevelTurnState::flightControl(StateContext& context) {
    ...
    if(count++ % 10 == 0)
    printf("LevelTurnState: angle=(%.2f, %.2f, %.2f), altitude=%.2f\n",context.angle[0], context.angle[1], context.angle[2], context.altitude);
} 

StateID LevelTurnState::isTransitionState(StateContext& context) {

    // 水平旋回状態からの遷移条件を確認し、必要に応じて次の状態IDを返す
